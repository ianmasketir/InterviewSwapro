...
namespace PORECT.Helper
{
	...
	{
		public Attachment(string xmlAttachment)
		{
			XDoc = XDocument.Parse(xmlAttachment);
			FileName = XDoc.Root.Element("name").Value;
			Base64Content = XDoc.Root.Element("content").Value;

			if (string.IsNullOrEmpty(Base64Content) || Base64Content.ToLower() == "scnull")
			{
				HasContent = false;
			}
			else
			{
				ByteArrayContent = Convert.FromBase64String(Base64Content);
				MemoryStreamContent = new MemoryStream(ByteArrayContent);
				FormFileContent = new FormFile(MemoryStreamContent, 0, MemoryStreamContent.Length, "Name", FileName);
				HasContent = true;
			}
		}
		public Attachment(string fileName, string base64)
		{
			string xmlAttachment = "<file><name>fileName</name><content>base64</content></file>";
			xmlAttachment = xmlAttachment.Replace("fileName", fileName).Replace("base64", base64);
		...
		public Attachment(IFormFile formFileContent)
		{
			...
			{
				HasContent = true;
				MemoryStreamContent = new MemoryStream();
				FormFileContent.CopyTo(MemoryStreamContent);
				ByteArrayContent = MemoryStreamContent.ToArray();
				Base64Content = Convert.ToBase64String(ByteArrayContent);
				FileName = FormFileContent.FileName;
				XDoc = XDocument.Parse("<file><name>" + FileName + "</name><content>" + Base64Content + "</content></file>");
			}
			else
			{
				HasContent = false;
		...
		public MemoryStream MemoryStreamContent { get; protected set; }
		public byte[] ByteArrayContent { get; protected set; }
		public string? FileName { get; private set; }
		public string? Base64Content { get; private set; }
		public IFormFile FormFileContent { get; private set; }
